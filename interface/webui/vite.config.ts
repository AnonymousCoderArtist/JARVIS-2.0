...
export default defineConfig(({ mode }) => {
  ...
  const target = env.JARVIS_API_URL ?? "http://127.0.0.1:8765";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../jarvis/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
    },
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the JARVIS ws upgrade both sit on
      ...
      proxy: {
        // WebSocket proxy for /jarvis/ws must come before the /jarvis HTTP proxy
        "/jarvis/ws": {
          target: wsTarget,
          ws: true,
          changeOrigin: true,
        },
        "/jarvis": { target, changeOrigin: true },
        "/api": { target, changeOrigin: true },
        "/auth": { target, changeOrigin: true },
        // Forward only WebSocket upgrades on ``/`` to the JARVIS gateway;
