...
export function ContextProgress({ open, onClose }: ContextProgressProps) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to load context usage");
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  const outPct = data?.limits?.output
    ? Math.min(100, ((data?.usage?.completion_tokens ?? 0) / data.limits.output) * 100)
    : 0;
        ...
        <div className="p-5 space-y-4">
          {loading && !data ? (
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              <Loader2 className="h-6 w-6 animate-spin" style={{ color: "rgba(var(--brand-r), var(--brand-g), var(--brand-b), 0.6)" }} />
              <span className="text-xs" style={{ color: "rgba(var(--text-muted-r), var(--text-muted-g), var(--text-muted-b), 0.5)" }}>Loading context usage...</span>
            </div>
          ) : error ? (
            <div className="text-center py-6">
              <p className="text-xs" style={{ color: "rgba(var(--error-r), var(--error-g), var(--error-b), 0.7)" }}>{error}</p>
              <button
                onClick={load}
                className="mt-3 text-xs px-3 py-1.5 rounded-lg"
                style={{
                  background: "rgba(var(--brand-r), var(--brand-g), var(--brand-b), 0.1)",
                  border: "1px solid rgba(var(--brand-r), var(--brand-g), var(--brand-b), 0.2)",
                  color: "rgba(var(--brand-r), var(--brand-g), var(--brand-b), 0.7)",
                }}
              >
                Retry
              </button>
