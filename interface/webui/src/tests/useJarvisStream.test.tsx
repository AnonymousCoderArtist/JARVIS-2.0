...
function wrap(client: ReturnType<typeof fakeClient>["client"]) {
  return function Wrapper({ children }: { children: ReactNode }) {
    return (
      <ClientProvider
        client={client as unknown as import("@/lib/jarvis-client").JarvisClient}
        token="tok"
      >
        {children}
      </ClientProvider>
...
describe("useJarvisStream", () => {
  it("starts in streaming mode when history shows pending tool calls", () => {
    ...
    const { result } = renderHook(
      () => useJarvisStream("chat-p", initialMessages, true),
      {
  ...
  it("collapses consecutive tool_hint frames into one trace row", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useJarvisStream("chat-t", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("attaches assistant media_urls to complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useJarvisStream("chat-m", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps assistant buttons on complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useJarvisStream("chat-q", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps streaming alive across stream_end and completes on turn_end", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useJarvisStream("chat-s", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
