...
The browser front-end for the jarvis gateway. It is built with Vite + React 18 +
...
jarvis/web/dist/      build output served by the gateway
```

## Develop from source

### 1. Install jarvis from source
...
```bash
jarvis_API_URL=http://127.0.0.1:9000 bun run dev
```
...
This writes the production assets to `../jarvis/web/dist`, which is the
directory served by `jarvis gateway` and bundled into the Python wheel.
