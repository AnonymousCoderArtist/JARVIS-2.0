...
from core.windows.desktop.utils import run_with_graceful_timeout, is_elevated
            ...
            while True:
                try:
                    ...
                    if reg_type == winreg.REG_EXPAND_SZ:
                        ...
                        # environment, which may be stripped by the JARVIS host. References
...
def _prepare_env() -> dict[str, str]:
    ...
    JARVIS hosts (e.g. Claude Desktop) may launch this server with a stripped
    ...
    try:
        ...
        # appended to fill in anything missing (e.g. stripped JARVIS host env).
    ...
    def execute_command(
        ...
        try:
            ...
            result = run_with_graceful_timeout(
                args,
                stdin=subprocess.DEVNULL,  # Prevent child processes from inheriting the JARVIS pipe stdin
            ...
            if result.returncode != 0 and "Access is denied" in output and not is_elevated():
                output += (
                    ...
                    "The JARVIS server is currently running at a lower integrity level."
                )
