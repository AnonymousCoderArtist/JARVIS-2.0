...
FIND_HF_API_TOOL_SPEC = {
    "name": "find_hf_api",
    "description": (
        "Find HuggingFace Hub REST API endpoints to make HTTP requests. Returns curl examples with authentication. "
        "Use this tool when you need to call the HF Hub API directly."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "Keyword search across endpoint summaries, descriptions, and operation IDs.",
            },
            "tag": {
                "type": "string",
...
class ExploreHfDocsTool(BaseTool):
    ...
    input_schema = EXPLORE_HF_DOCS_TOOL_SPEC["parameters"]

    async def execute(self, input_data: ToolInput) -> ToolOutput:
        args: dict[str, Any] = {}
        for key in ("endpoint", "query", "max_results"):
            val = getattr(input_data, key, None)
            if val is not None:
                args[key] = val
        if "endpoint" not in args:
            return ToolOutput(success=False, result=None, error="endpoint is required")

        class _Session:
            hf_token = os.environ.get("HF_TOKEN")

        try:
            result, success = await explore_hf_docs_handler(args, session=_Session())
...
class FetchHfDocsTool(BaseTool):
    ...
    input_schema = HF_DOCS_FETCH_TOOL_SPEC["parameters"]

    async def execute(self, input_data: ToolInput) -> ToolOutput:
        url = getattr(input_data, "url", None)
        if not url:
            return ToolOutput(success=False, result=None, error="url is required")

        class _Session:
            hf_token = os.environ.get("HF_TOKEN")

        try:
            result, success = await hf_docs_fetch_handler({"url": url}, session=_Session())
            return ToolOutput(success=success, result=result)
        except Exception as e:
            return ToolOutput(success=False, result=None, error=f"Error: {e}")


class FindHfApiTool(BaseTool):
    name = FIND_HF_API_TOOL_SPEC["name"]
    description = FIND_HF_API_TOOL_SPEC["description"]
    input_schema = FIND_HF_API_TOOL_SPEC["parameters"]

    async def execute(self, input_data: ToolInput) -> ToolOutput:
        args: dict[str, Any] = {}
        for key in ("query", "tag"):
            val = getattr(input_data, key, None)
            if val is not None:
                args[key] = val
        if not args:
            return ToolOutput(success=False, result=None, error="Provide query or tag")

        try:
            result, success = await search_openapi_handler(args)
            return ToolOutput(success=success, result=result)
        except Exception as e:
            return ToolOutput(success=False, result=None, error=f"Error: {e}")
