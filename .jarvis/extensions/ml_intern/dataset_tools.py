...
class HfInspectDatasetTool(BaseTool):
    ...
    input_schema = HF_INSPECT_DATASET_TOOL_SPEC["parameters"]

    async def execute(self, input_data: ToolInput) -> ToolOutput:
        import os

        args: dict[str, Any] = {}
        for key in ("dataset", "config", "split", "sample_rows"):
        ...
        try:
            result, success = await hf_inspect_dataset_handler(args, session=_Session())
