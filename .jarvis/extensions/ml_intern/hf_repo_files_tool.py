...
async def hf_repo_files_handler(
    ...
    try:
        ...
        tool = _HfRepoFilesInternal(hf_token=hf_token, session=session)
...
class HfRepoFilesTool(BaseTool):
    ...
    input_schema = HF_REPO_FILES_TOOL_SPEC["parameters"]

    async def execute(self, input_data: ToolInput) -> ToolOutput:
        args: Dict[str, Any] = {}
        for key in ("operation", "repo_id", "repo_type", "revision", "path", "content", "patterns", "create_pr", "commit_message"):
        ...
        try:
            result, success = await hf_repo_files_handler(args, session=_Session())
